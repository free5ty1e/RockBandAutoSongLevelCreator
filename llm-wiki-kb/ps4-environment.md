...
- **Our dev container CAN run ForgeTool.** The devcontainer Dockerfile installs `mono-devel` + `libgdiplus` (apt) + the .NET SDK 8 to `/tmp/dotnet`, and `.devcontainer/post-install.sh` runs `tools/build_forgetool.sh`. That script now auto-detects mono's reference-assembly dir (Linux `/usr/lib/mono/4.7.1-api`, macOS Homebrew `$(brew --prefix mono)/lib/mono/4.7.1-api` on Intel & Apple Silicon, or old `/Library/Frameworks/Mono.framework/...`) and checks for `libgdiplus` (ForgeTool's System.Drawing needs it to read the CON's album art), so CON → PKG conversion can be validated locally on both OSes (verified: `ForgeTool.exe con2gp4` converted `output/open_road_song.con` → GP4 successfully). **macOS note:** Homebrew's `mono` does not bundle libgdiplus — install it separately with `brew install mono-libgdiplus`, or ForgeTool fails at conversion with `DllNotFoundException: libgdiplus.dylib`. Installing it is not enough by itself: mono 6.x does not search Homebrew's lib dir (`/opt/homebrew/lib` on Apple Silicon) for native deps by default, so `tools/forgetool` now sets `DYLD_FALLBACK_LIBRARY_PATH` to `$(brew --prefix)/lib` on macOS so libgdiplus (and its pango/cairo deps) resolve.
