...
- **Track/channel layout must match the physical MOGG (v0.0059).** `MakeMoggDta` splits each dta `tracks` entry into per-channel `(instrument (ch...))` groups (drum 4ch → `drum (0)` `drum (1)` `drum (2 3)`), derives `lastTrack` from the last non-empty entry, and appends `fake` tracks for any trailing channels, e.g. 10 physical channels ending at track 8 → `fake (9)`. `maxton/LibForge#30` ("Mismatching audio track and instruments") documents that a track/channel assignment that does not match the physical MOGG makes songs **start but stop playing after a few seconds in-game** — the failure family of the persistent "no preview + instant 0%" bug. AutoRB therefore builds a 10-channel MOGG and 311-compatible dta so `MakeMoggDta` emits the identical track list as the known-good "311 - Down": `drum (0) drum (1) drum (2 3) bass (4) guitar (5 6) vocals (7 8) fake (9)`.
- **MIDI chart must carry EVENTS markers + a real BEAT track (v0.0061).** `RBMidConverter` matches tracks by name and ignores `EVENTS`/`BEAT`, but the conversion pipeline and in-game behavior depend on them: stock RB3 charts always carry `[prc_intro]`, `[music_start]`, `[prc_verse_1]`, `[preview]`, `[prc_chorus_1]`, `[prc_outro]`, `[music_end]`, `[end]` (final event) in the EVENTS track and one quarter-note marker per beat in BEAT (pitch 12 downbeat vel 101, pitch 13 vel 100, 480-tick spacing). A missing `[preview]` kills the song-list preview; a missing `[music_end]`/`[end]` makes the song finish instantly at 0%.
- **Vocal phrase close must be a real `note_off` (v0.0063 finding).** The tested PKG's chart closed each phrase with `note_on 105 vel 0` (0x90/00) instead of `note_off 105` (0x80/00) — 18 opens, 0 real closes, all 284 notes outside any phrase, so vocal scoring silently did nothing. ForgeTool conversion still succeeds (RBMidConverter only needs the difficulty slots non-null), so this only shows up in-game as zero scoring. Regenerating from current code yields 59 opens == 59 closes.
- **First phrase must be past tick 640, or `StartTicks - 640` underflows a `uint` (v0.0064 finding).** `RBMidConverter.cs:1286` computes phrase StartMillis via `e.StartTicks - 640` on a `uint`. A chart whose first phrase marker is at tick 311 (pre-count-in charts started content at tick 0) wraps to `4294966967`, so `GetMillis` returns end-of-song (~198s), the first phrase's StartMillis lands at the end with a negative length, and the vocal guide breaks (`RBMidWriter.cs:197` writes StartMillis/LengthMillis/StartTicks/LengthTicks verbatim). The constant +640 cannot be baked in because `MidiHelper.cs:23-31` does `idx--` on the tempo map and crashes with index -1 if there's no tick-0 tempo entry. The v0.0064 count-in solves it structurally: the first phrase now sits at tick 6071, so `6071 - 640 = 5431` is safe.
- **`HasFreestyleVocals` in `songdta_ps4` is hardcoded `false` upstream — AutoRB patches it (v0.0067).** The PS4 `songdta_ps4` binary format has no slot for `vocal_tonic_note`/`song_tonality`; `SongDataConverter.ToSongData` silently drops them, so they cannot drive RB4's freestyle guide lines. But the format *does* carry a `HasFreestyleVocals` bool (`SongData.cs:31`, written at `SongDataWriter.cs:42`), and upstream ForgeTool hardcoded it to `false` (`SongDataConverter.cs`), meaning no ForgeTool-converted custom ever advertised freestyle support to the game. The vendored source is patched so `SongDataConverter` reads `HasFreestyleVocals = songDta.Array("freestyle_vocals")?.Int(1) == 1` — AutoRB writes `(freestyle_vocals 1)` into `songs.dta` only under the `--generate-freestyle-vocals` flag. Verified end-to-end by parsing the built `songdta_ps4`: flag byte = 1 with the flag, 0 without. Because ForgeTool is vendored as **source** (`tools/libforge`) and rebuilt by `.devcontainer/post-install.sh` → `tools/build_forgetool.sh` on every fresh container (and by any fresh `git clone` + build for wheel users, who don't get ForgeTool binaries in the wheel), the patch propagates automatically — no binary distribution is needed.
