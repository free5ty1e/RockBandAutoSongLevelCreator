...
## Overlap-Free Note Ends (v0.0072)
The sparse tempo map (v0.0071) was necessary but not sufficient — the in-game progressive drift persisted because the bug was in **note emission**, not tempo. `synced_track.json` word `end` times (stretched across multi-syllable phrases) run past the next word's start ("Tonight" ends 0.94s, "I" starts 0.85s); `generate_vocal_midi()` emitted each note at `max(target_start, previous_end)`, so overlapping pairs pushed later notes progressively later (charted-vs-whisper error grew 0 → 5.2s → 8.4s). The fix precomputes all charted start/end ticks and **clips each note's end to the next note's charted start**, keeping every `note_on` exactly at its charted onset. Lesson: verify charted note *positions* against content ground truth (whisper/LRC word times); onset-vs-onset matching masks progressive drift because musical beats repeat.
...
## Default Album Art Legibility + CP Logo (v0.0075)
The default "Chris Prime Custom" album art (`autorb/export/album_art.py`) was illegible on macOS because it hardcoded the Linux-only DejaVu font path `/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf`. On macOS that path is absent, so the font fell back to Pillow's size-10 bitmap default: "CHRIS" rendered ~124 glyph pixels vs DejaVu's ~1858. The devcontainer had DejaVu installed, masking the bug. Fix: DejaVu Sans Bold/Regular (permissive Bitstream Vera license) are now **bundled in the wheel** (`autorb/export/data/fonts/`, shipped via `[tool.setuptools.package-data]`). `_load_font()` prefers the bundled copy, then known OS paths (Homebrew/Library/macOS, `/usr/share/fonts/...`/Linux), then Pillow's scalable `load_default(size=)` (Pillow ≥ 10.1). A circular "CP" monogram logo was added to the bottom-right corner (orange ring + white C/P strokes). New regression tests: `test_album_art_title_is_legible` (asserts thousands of white glyph px on 256px art) and `test_album_art_has_cp_logo` (asserts orange ring + white glyphs in bottom-right). Full suite: 42 passed.
