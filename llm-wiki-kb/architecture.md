...
6. **STFS/CON Packaging & Validation:** `con_packer.py` assembles the hierarchical `songs/{song_id}/` structure containing `.mid`, `.mogg`, `songs.dta` (with full 4-stem multitrack mapping: drums `0-1`, bass `2-3`, guitar `4-5`, vocals `6-7`), plus `gen/` milo + PNG assets re-used from the signed `SmellsLikeNirvana_rb3con` template. The `.mid` chart (built by `midi_generator.py`) always contains `BEAT`, `EVENTS`, `PART VOCALS`, and placeholder `PART DRUMS` / `PART GUITAR` / `PART BASS` tracks (one note each at pitch 60) so every instrument advertised in `songs.dta` has a loadable chart track. Payloads are placed using the STFS logical->physical block mapping (see [con_stfs_format.md](con_stfs_format.md)); the staged milo is the true template milo and is verified parseable by a LibForge replication (see [forgetool_compat.md](forgetool_compat.md)). `stfs_validator.py`, `forge_simulator.py`, and `pytest` provide automated validation of STFS file tables, parent directory references, and payload placement.

## Known Crash & Fix Notes
- **RB4 vocal-fretboard crash (v0.0056):** After the MOGG fix, RB4 still crashed the moment the vocal fretboard loaded. Working hypothesis: `songs.dta` advertises `PART DRUMS` / `PART GUITAR` / `PART BASS` at rank 150 but the chart contained only `PART VOCALS`, so RB4 (via ForgeTool PKG conversion) had nothing to load for the advertised parts. Fix: `generate_vocal_midi()` now emits placeholder instrument tracks (one note at pitch 60) so every advertised part resolves to a real MIDI track.
