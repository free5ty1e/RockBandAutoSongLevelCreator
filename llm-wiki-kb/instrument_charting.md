...
How AutoRB turns separated stems into playable 5-lane instrument charts (guitar, bass, drums, keys) and how those charts are serialized to MIDI and to the Clone Hero `.chart` format. Captured at **v0.0.92** (first alpha with *real*, non-placeholder instrument charts); drum element classification + fill detection reworked in **v0.0.94**; chord (multi-pitch) transcription + simultaneous-note MIDI fix in **v0.0.98**; bass-merge / guitar-chord-overdetection / drum-difficulty fixes in **v0.0.99**; Clone Hero drum-flattening fix + Expert double-bass reducer in **v0.1.0** (difficulty-derivation rules documented in `[[difficulty_charting]]`); difficulty-derivation rewrite in **v0.1.1**; illegal same-lane-overlap fix in **v0.1.2**; drum onset fix + guitar over-charting reduction + bass frequency filter in **v0.1.3**; **v0.1.4** massive guitar chord cleanup + drum balance + bass hold extension; **v0.1.5** intro drums from the mixed audio + classifier hierarchy fix (measured on the WRONG audio — superseded, see below); **v0.1.6** correct test track + ADTOF neural drums + RBN drum difficulty rules + guitar chord reconstruction; **v0.1.7** tom lane assignment fixed (pitch + ring-energy split; fills hit Green/Blue pads, hi-hat lane depacked); **v0.1.8** bass energy gate fixed (whole-stem global RMS → two-stage bass-band (40–250 Hz) RMS gate vs active level + density isolation; removes all intro bleed, keeps soft real bass); **v0.1.9** drift-correct tempo grid (fixes "charted notes off the beat" — snapping now interpolates from the tempo_map's per-beat times instead of a uniform t=0 grid, so bass/guitar/drums land `median=0 ms` on the true beat grid); bass transcribes at 1/8 grain; guitar held-chord merge reverted to the safe per-gap version (the v0.1.8 region-collapse over-merged the whole part to n=1 under bass-bleed-in-the-other-stem; the per-gap chroma+envelope-gated merge reduces the bridge 53→23 without collapsing it); **v0.1.10** opt-in `--use-ft-stems` (chunked `htdemucs_ft`, 10 s OLA chunks + OOM→6 s→stock-`htdemucs` fallback; slow on CPU, GPU or master stems); bridge chord-identity analysis — per-strum root flips 36×(stock)/41×(ft) so no per-strum merge reaches 4, but a 3–4 s windowed majority recovers 3 chords (D/E/A, 4 changes) → the bridge is a short power-chord progression, not one held chord; a windowed-chord-identity merge is a future item, never hand-authored. **v0.1.11** sectioned `htdemucs_ft` into 45 s overlapping strips (level-constant strip crossfade) so the full 198 s run completes on an 8 GB CPU (the chunked path's `sources_p` accumulator was sized to the full input and OOM-killed at chunk 4); reflection-pad boundary-spike fix in the sine-window OLA reconstructor. **v0.1.12** the ft ensemble is no longer unwrapped to a single sub-model (that sub-model bleeds bass↔other 5× worse, NCC 0.172 vs 0.034); the full `BagOfModels` ensemble now runs through the strip path at ~2.3 GB peak RSS on 8 GB. New knobs exposed: `--ft-shifts` (default 1; shifts=2 verified ~no gain), `--ft-strip-seconds` (default 45), `--ft-segment` (default None = full context). Experiment results in `[[log]]`. **v0.1.13** `--use-ft-stems` is now DEFAULT ON (was opt-in flag); `separate_stems()` default `model_name` changed to `htdemucs_ft`; new `--ft-overlap` knob (default 0.25; 0.5 = cleaner transients at ~2× time/RAM). **v0.1.14** new `--separator` CLI option (default `htdemucs_ft`; choices: `htdemucs`, `htdemucs_6s` (6 stems incl. guitar+piano), `spleeter:5stems` (5 stems incl. piano)); `separate_stems_spleeter()` added; `--use-ft-stems`/`--no-use-ft-stems` deprecated aliases; full docs in `[[piano_keyboard_separation]]`. **v0.1.15** dedicated `guitar.wav`/`piano.wav` now drive guitar/keys charts (and are mixed into MOGG ch5-6); stems dir auto-cleared on every separation. **v0.1.16** guitar transcription tuned for 1/8-note rhythm parts: Basic Pitch ONSET 0.45->0.40, FRAME 0.35->0.30; `_frag_gap` changed from fixed 1.5s/2.0s to `min(3 x eighth_note_at_bpm, 2.0)` s (old window swallowed ~8 consecutive 8th strums at 169 BPM). Verified: strum count 555->659, audio coverage 56%->102%, 98.6% within 200ms of grid. **v0.1.13** `--use-ft-stems` is now DEFAULT ON (was opt-in); the full ensemble + 45 s strips is the standard separator, with `--no-use-ft-stems` for the fast-but-lower-quality stock `htdemucs`. `--ft-overlap` added (default 0.25; 0.5 = cleaner transients, 2× time/RAM). `separate_stems()` default `model_name` changed to `"htdemucs_ft"`. User listening test confirmed ft beats stock across all stems (drums, bass, other/rhythm-guitar, vocals). See `[[log]]`.

### v0.1.9 — drift-correct tempo grid (fixes "charted notes off the beat")

`_snap` / `_on_grid` in `autorb/transcribe/instruments/difficulty.py` snapped every note to a uniform grid anchored at **t = 0** (`round(time / step) * step`), with `step = 60 / (bpm * subdivisions)` using a single representative BPM. Under Eve6's per-beat tempo map the BPM drifts 152 ↔ 184 (~18 % over the track), so that uniform grid diverged from the **real** beats and near-beat attacks landed on the wrong 1/16 tick — the "off-the-beat" symptom carried over from the v0.0.92 reference (notes ticked from the *inverse* of a jittery per-beat map and drifted progressively).

`difficulty.py::_on_grid` / `::snap_to_grid` now take `tempo_map` and return a **beat-interpolated instant**: each target `time` is interpolated from the tempo_map's per-beat `(beat_start, bpm)` sequence (per-beat dynamic BPM), so chart and map are self-consistent by construction (a note's tick = inverse of the grid the MIDI carries). The local BPM for any instant comes from `tempo_map.bpm_at(time)` (bisect of `beat_times` → `bpm`).

**Verified on "Open Road Song"** (regen, `--skip-separation --skip-tempo-detection --skip-vocals`, 198 s, 534 beats, BPM 152↔184):
- `output/validation/notes.mid` timing: **bass median = 0 ms (1/615 > 44 ms), guitar median = 0 ms (14/829), drums median = 0 ms (3/1316)** off the true beat grid — every instrument fixed at once.
- 0 same-lane sustain overlaps at all difficulties.
- Bass Expert 615 (first note 23.25 s / measure 17, 0 pre-m17 bleed, 1/8 grain); guitar Expert 829; drums Expert 1316.

### v0.1.9 — guitar held-chord merge reverted to the safe per-gap version

The v0.1.8 region-collapse rewrite was intended to consolidate a sustained power chord re-emitted every ~178 ms into one held note, but on Eve6 it **over-merged**: `min(pitch)` (the root-segmented key) is a near-constant low note because the Demucs `other` stem carries bass bleed, so every region shared one root and the entire guitar Expert collapsed to **`n = 1`** (`notes=1`, `bridge notes=0`). Reverted to the conservative per-gap merge: fuse consecutive chord groups within `_frag_gap = 2.0 s` iff (a) chroma-stable (`_stable` — argmax of `chroma_stft` differs at a real chord change, so different chords never merge), (b) envelope-continuous (`_held` — RMS min ≥ 15 % of local peak between attacks, which breaks the chain at phrase rests and makes an n=1 collapse impossible), and (c) not the identical `(lane, is_open)` key requirement (dropped — Basic Pitch + the power-fifth addition pick different strings per re-emission). Bridge 53 → **23** fragments (not 4 — see bridge = exactly 4 is not recoverable), intro riff 51 preserved, br2 strumming 119 intact, 0 overlaps.

See `[[log]]` and the v0.1.9 CHANGELOG entry.
...
### v0.1.8 — bass energy gate (two-stage)
The v0.0.99 bass frequency filter (38–420 Hz) and the v0.1.7 bass path are correct in range, but the *phantom bass in the intro* survived: the energy gate in `_transcribe_fretted` (bass branch) compared each note's energy to **whole-stem global RMS**, which collapses toward zero across the long silent intro (bass `bass.wav` is digitally silent until ~23 s on "Open Road Song"). With the global mean dragged down, the "2% of global RMS" test let Demucs bleed walk through — 12 phantom bass notes before the real first note at measure 17 (including a 13.3 s A2 spike at band RMS 0.032 that sits in the same energy band as soft real bass).

**Fix** (`autorb/transcribe/instruments/guitar.py`, bass branch only):
1. **Band-pass** the stem to 40–250 Hz (the bass's true energy band, not the full-band RMS that lets hi-hat/guitar bleed inflate the number).
2. **Stage 1 — hard floor** at 35% of the band RMS 75th percentile (the track's own *active* level, not the whole-song mean). Removes the quiet intro/0–16 s bleed (band RMS 0.01–0.022).
3. **Stage 2 — density guard.** The 13.3 s A2 bleed (0.032) is above stage 1's floor, so it survives on raw energy alone. It is dropped by a density pass: in the ambiguous band [floor, 0.5×active), a note is kept only if a **neighboring** bass note exists within 0.5 s. Real bass is rhythmically dense (≤0.5 s between onsets at this tempo); the 13.3 s spike's only surviving neighbor is the real first note 10 s later, so it is flagged phantom. Dense *soft* real bass (eighth-note or faster) is preserved — no real notes lost.

**Verified on "Open Road Song"** (`output/validation/notes.mid`, regenerated with `--skip-separation --skip-tempo-detection --skip-vocals`): bass Expert **612 notes, 0 notes before the genuine first bass note at 23.25 s (measure 17)**; 23.25 s note retained (band RMS 0.111). `git diff` = `guitar.py` only; guitar/keys/drums paths untouched. See `[[vocal_alignment]]` for the parallel (unrelated) vocal-sync test failures that are pre-existing.

verified on the Eve6 test track: bass Expert **612 notes, 0 notes before the genuine first bass note at 23.25 s (measure 17)**; bass-band (40–250 Hz) RMS 75th pctile 0.071, floor 0.0245 (35%), ambiguous-band upper 0.0357 (0.5×). See `[[vocal_alignment]]` for the parallel (unrelated) vocal-sync test failures that are pre-existing.

### v0.1.8 — guitar held-chord fragment merge

`_transcribe_fretted` (guitar branch) now collapses held-chord strum-fragments: a sustained power chord strummed an 8th-note is re-emitted by Basic Pitch every ~178 ms @ 169 BPM, reading as a "mass of overlapping chords" / "chopped into tiny fragments." New step 3c merges chord groups within ~1 beat (tempo-relative 0.5 s) that share the identical `(lane, is_open)` set, are **chroma-stable** (no pitch-class change → not a new chord), and **envelope-continuous** (RMS ≥ 15% of peak between attacks → a genuine hold, not a damped strum). Damped 8th-strumming (envelope dips) and rapid distinct-chord changes (chroma flip) are preserved. On Eve6 this consolidated guitar Expert 1489→1334 notes with **0 same-lane overlaps**; the bridge strum rhythm (br2 203 attacks) was untouched. The envelope floor must be tuned per song if a part uses deliberately muted single-lane 8th patterns.
...
Keys reuses the guitar transcription because, with the default `htdemucs`/`htdemucs_ft` separator, Demucs' `other` stem is guitar+keys inseparable (see `[[architecture]]` and `[[piano_keyboard_separation]]`). **As of v0.1.14**, the `--separator htdemucs_6s` option provides a separate `piano` stem and a separate `guitar` stem (splitting guitar out of `other`), and `--separator spleeter:5stems` provides a separate `piano` stem. See `[[piano_keyboard_separation]]` for the full comparison.
