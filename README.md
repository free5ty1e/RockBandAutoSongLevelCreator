...
* **Smart Lyrics & Vocal Alignment:** 
  ...
  * **Onset-snapped timing:** WhisperX word boundaries are systematically late (median ~80ms, tail ~400ms), and Basic-Pitch's own note onsets can lag the true sung attack by another ~300ms (measured: the first word of "Open Road Song" was charted ~350ms late). Each word's start is now snapped to the **earliest voiced vocal-stem attack onset** (librosa onset detection on the vocal stem, filtered to onsets with a pyin-confident voiced frame within 0.25s) inside the search window — the true sung onset, never an unvoiced consonant or the pre-sound silence floor (backtracking to the envelope floor once charted the first word 0.38s before its LRC timestamp) and never back into the previous word or beyond its own window, and multi-syllable word ends extend across all notes inside the word's own span. **The charted note lands on that snapped word start**, not Basic-Pitch's pitch onset: `generate_vocal_midi()` anchors the first note of each word at `word.start` (BP pitch onsets can lag the attack by 0.1-0.6s and, on held words, land near the word's *end*), while later segments within the word keep their own pitch-change times so syllable-internal slides and vibrato survive.
  * **Overlap-free note ends:** a word's stretched end time frequently runs past the *next* word's start ("Tonight" ends at 0.94s while "I" starts at 0.85s). If each note's duration is emitted as-is, every overlapping pair pushes the following note later and the pushes accumulate — charted notes drift progressively later than the audio (first word right-on, then late), which is the PS4 symptom that survived the v0.0071 tempo-map change. Each note's duration is now **clipped to the next note's charted start**, so every `note_on` lands exactly on its true sung onset and consecutive notes never overlap. The **last word of each LRC line** (phrase source of truth) additionally sustains toward the next line's timestamp minus a 0.10s breath gap, so held final syllables ("forgottennnn") are never chopped at WhisperX's word boundary; line membership uses the raw pre-snap start so a phrase's first word snapped before its LRC timestamp isn't mistaken for the previous line's last word.
