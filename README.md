...
* **Smart Lyrics & Vocal Alignment:** 
  ...
  * **Audio-derived word starts & ends:** LRC/WhisperX timestamps are **suggestions only** — the MP3 and LRC come from different sources, so they carry a global offset and WhisperX word boundaries run ~80-400ms late. Each word's **start** is snapped to the latest true vocal-stem attack onset at-or-before its WhisperX boundary (librosa onset detection on the vocal stem, kept when a pyin-confident voiced frame or an RMS energy rise follows it within the search window, backtracking to the envelope floor once charted the first word 0.38s before its LRC timestamp and never back into the previous word or beyond its own window), and each word's **end** is the last voiced/RMS-energetic frame of its own region — so over-sustains are clipped ("bored" 22.8s→21.0s, "mirror" 34.6s→32.9s, "floor" 11.5s→9.9s) and under-sustains extend ("forgottennnn" now reaches its true sung tail). **The charted note lands on that snapped word start**, not Basic-Pitch's pitch onset: `generate_vocal_midi()` anchors the first note of each word at `word.start` (BP pitch onsets can lag the attack by 0.1-0.6s and, on held words, land near the word's *end*), while later segments within the word keep their own pitch-change times so syllable-internal slides and vibrato survive.
...
# pyproject.toml, and CHANGELOG.md — currently 0.0.85)
git tag v0.0.85
