...
def _separate_strips(model, wav, sr, device,
    ...
    **Memory-bounded design:** This function writes each stem directly to its output
    ...
    if output_stems_dir is not None and stem_names is not None:
        ...
        strip_temp_files = []  # list of lists: [source][strip] = temp_file_path

        # Create temp file for each strip of each source
        ...
        try:
            ...
            for i, s in enumerate(starts):
                ...
                for src_idx in range(n_src):
                    ...
                    temp_file.close()
                    strip_temp_files[src_idx].append(temp_path)

                    with sf.SoundFile(temp_path, mode='w', samplerate=sr,
                                      channels=ch, subtype='FLOAT', format='WAV') as sf_handle:
                        sf_handle.write(strip_data)
            ...
            for src_idx in range(n_src):
                for temp_path in strip_temp_files[src_idx]:
                    try:
                        os.unlink(temp_path)
                    except OSError:
                        pass
